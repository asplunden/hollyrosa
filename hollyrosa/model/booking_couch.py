...
def get_bookings_of_visiting_group(holly_couch, visiting_group_id, visiting_group_name):
    args = [visiting_group_id, visiting_group_name]
    return holly_couch.view('visiting_groups/bookings_of_visiting_group', keys=[k for k in args if k != None],
                            include_docs=True)
...
def getBookingDays(holly_couch, from_date='2011-01-01', to_date='3015-12-11'):
...
def getVisitingGroupsByBoknstatus(holly_couch, status):
    return holly_couch.view("visiting_groups/all_visiting_groups_by_boknstatus", startkey=[status, None],
                            endkey=[status, '9999-99-99'], include_docs=True)  # keys=[[s, None]  for s in statuses],
...
def getVisitingGroupsByVodbState(holly_couch, state):
    return holly_couch.view("visiting_groups/all_visiting_groups_by_vodb_state", startkey=[state, None],
                            endkey=[state, '9999-99-99'], include_docs=True)


def getVisitingGroupsByGroupType(holly_couch, group_type):
    return holly_couch.view("visiting_groups/all_visiting_groups_by_group_type", startkey=[group_type, None],
                            endkey=[group_type, '9999-99-99'], include_docs=True)
...
def getAllHistoryForVisitingGroup(holly_couch, visiting_group_id, limit=None):
    ...
    return holly_couch.view('history/all_history_by_visiting_group',
                            startkey=[visiting_group_id, 'zzzzzzzzzzzzzzzzzzzzzzzzzzzzz'],
                            endkey=[visiting_group_id, None], include_docs=True, descending=True, limit=limit)


def getAllHistoryForUser(holly_couch, user_id, limit=None):
    """returns booking history sorted in reverse chronological order."""
    return holly_couch.view('history/history_by_username', keys=[user_id], include_docs=True, descending=True,
                            limit=limit)


def getAllHistoryForBookings(holly_couch, booking_ids, limit=250):
    """returns booking history sorted in reverse chronological order."""
    return holly_couch.view('history/history_by_booking_id', keys=booking_ids, include_docs=True, descending=True,
                            limit=limit)
...
def getTagStatistics(holly_couch, group_level=999, startkey=None):
    if startkey == None:
        startkey = []
    return holly_couch.view('tag_statistics/tag_group_statistics', startkey=startkey, reduce=True,
...
def gelAllBookingsWithBookingState(holly_couch, booking_states, limit=200):
    return holly_couch.view('workflow/all_bookings_by_booking_state', keys=booking_states, include_docs=True,
                            limit=limit)
...
def getSchemaSlotActivityMap(holly_couch, booking_day, subtype):
    ...
    try:
        ...
        for a in res:
            ...
            m[a.key[0]] = dict(activity_id=v[0], duration=v[1]['duration'], time_to=v[1]['time_to'],
                               slot_id=v[1]['slot_id'], time_from=v[1]['time_from'], title=v[1]['title'],
                               pref=v[1].get('pref', 'time'))
...
def getNotesForTarget(holly_couch, target_id):
    ...
    """
    return holly_couch.view("notes/notes_by_target_datesorted", include_docs=True, startkey=[target_id, None],
                            endkey=[target_id, "9999-99-99 99:99"])
...
def get_booking_info_notes_with_matched_language(holly_couch, used_activities_keys, visiting_group_language):
    """
    Fetches the booking info notes associated with used_activity_keys and arranges so they are returned in a way that matches visiting_group_language.
    :param holly_couch:
    :param used_activities_keys:
    :param visiting_group_language:
    :return:
    """
    ...
    for activity_id, note_by_lang in booking_info_notes.items():
        ...
        # best match: the specified language exists
    ...
    return booking_info_notes_with_matched_language
...
def getDocumentsByTag(holly_couch, tag):
    return holly_couch.view("tags/documents_by_tag", include_docs=True,
                            keys=[tag])  # startkey=[target_id, None], endkey=[target_id, "9999-99-99 99:99"])
...
def getAllActiveUsers(holly_couch, show_deactive=False):
    return holly_couch.view('user/all_users', include_docs=True, startkey=[(not show_deactive), ''],
                            endkey=[True, 'zzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzz'])
...
def getBookingOverview(holly_couch, from_date, to_date, group_level=9999, reduce=True):
    if reduce:
        ...
        return holly_couch.view('vodb_overview/vodb_overview', reduce=reduce,
                                include_docs=False)  # dont include docs, it gets really slow
...
def getBookingEatOverview(holly_couch, from_date, to_date, group_level=9999, reduce=True):
    if reduce:
        ...
        return holly_couch.view('vodb_overview/vodb_eat_overview', reduce=reduce,
                                include_docs=False)  # dont include docs, it gets really slow


def getRoomBookingsOfVODBGroup(holly_couch, visiting_group_id):
    return [b.doc for b in holly_couch.view('visiting_groups/live_bookings_of_visiting_group', include_docs=True,
                                            startkey=[visiting_group_id, ''],
                                            endkey=[visiting_group_id, 'zzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzz'])]
...
def getProgramLayerBucketTextByDayAndTime(holly_couch, visiting_group_id, booking_day_id, bucket_time):
    return holly_couch.view('program_layer/bucket_text_by_day_and_time', include_docs=True,
                            keys=[(visiting_group_id, booking_day_id, bucket_time)])
