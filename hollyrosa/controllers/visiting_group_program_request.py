...
from hollyrosa.controllers.common import getLoggedInUserId, computeCacheContent, has_level, reFormatDate, \
    bokn_status_map, \
    ensurePostRequest
...
validation_error_explanations = {
    'date_format': u"Det har blivit fel i formateringen av ett datum. Datum måste vara på formen YYYY-MM-DD. Det " \
                   u"kan bero på att ett datum inte angivits, men troligare är det ett fel vi programmerare har " \
                   u"gjort. ",
    'date_range': u"Ett datum som angivits är inte innom rimliga gränser. Datumet kan exempelvis vara innan er " \
                  u"grupp kommit till ön eller efter att den åkt hem igen. ",
    'date_order': u"Ett datum-par, oftast från-datum och till-datum, är i fel kronologisk ordning. Det är lite " \
                  u"orimligt att man åker hem innan man kommit ut till ön. ",
    'req_fm_em': u"En rad bland bokningsförfrågningarna saknar val av tid på dagen (fm/em/kväll)"}
...
class ValidationErrorMessages(list):
    """This class is used in the external booking request to do custom validation of errors found in the rather
    complex data sent from the grids. Typically, we have two dates and there are constraints on them. """

    def __init__(self):
        """The idea about explanations is to have a general explanation that can be used repeatedly but only shown
        once """
...
class VisitingGroupProgramRequest(BaseController):
    ...
    def edit_request(self, visiting_group_id=''):
        ...
        program_request_data_dict = {'identifier': 'id', 'items': [
            {'id': str(i), 'requested_date': visiting_group_o['from_date'], 'requested_time': '',
             'requested_activity': '', 'age_sma': False, 'age_spar': False, 'age_uppt': False, 'age_aven': False,
             'age_utm': False, 'age_rov': False, 'age_led': False, 'note': ''} for i in range(35)]}
        ...
        return dict(visiting_group_program_request=visiting_group_o, reFormatDate=reFormatDate,
                    bokn_status_map=bokn_status_map, hasValidationErrors=hasValidationErrors)

    @expose()
    @require(Any(has_level('pl'), has_level('vgroup'), msg=u'Du måste vara inloggad för att få spara programönskemål'))
    def update_visiting_group_program_request(self, program_request_info='', contact_person='', contact_person_email='',
                                              contact_person_phone='', vgroup_id='', program_request_input='',
                                              have_skippers=False, miniscout=False, ready_to_process=False,
                                              age_group_input='', saveButton='', submitButton=''):
        # TODO: once property data has changed, we somehow need to propagate that property data to all places where
        #  the properties are used BUT I dont think we really waht to propagate it unless we do some state-change,
        #  we probably want to check the data before it is allowed to progress into other parts of the system. TODO:
        #   We lost a lot of work, but our aim is to introduce the validation_error_messages list to contain all
        #   problems we have encountered. what about making a subclassed list which can add speciealy formatted
        #   messages? also, we should hav a comprehensive from-date-to-date checker
        #
        ...
        if may_change_request_data:
            ...
            for tmp_age_group in age_group_data_items:
                ...
                property_id = None
                for tmp_vgroup_property_id, tmp_vgroup_property in visiting_group_o[
                    'visiting_group_properties'].items():
                ...
                if property_found:
                    ...
                    ok_1, tmp_vgroup_property['from_date'] = sanitizeDate(tmp_age_group['from_date'],
                                                                          default_date=tmp_vgroup_property['from_date'])
                    if not ok_1:
                        validation_error_messages.report('properties',
                                                         u'Från-datum som tillhör åldersgrupp %s har fel format.' %
                                                         tmp_vgroup_property['property'],
                                                         problematic_value=tmp_age_group['from_date'],
                                                         explanation_key='date_format')
                    ok_2, tmp_vgroup_property['to_date'] = sanitizeDate(tmp_age_group['to_date'],
                                                                        default_date=tmp_vgroup_property['to_date'])
                    if not ok_2:
                        validation_error_messages.report('properties',
                                                         u'Till-datum som tillhör åldersgrupp %s har fel format.' %
                                                         tmp_vgroup_property['property'],
                                                         problematic_value=tmp_age_group['to_date'],
                                                         explanation_key='date_format')
                    ok_3 = (tmp_vgroup_property['to_date'] >= tmp_vgroup_property['from_date'])
                    if not ok_3:
                        validation_error_messages.report('properties',
                                                         u'Till-datum kan inte inträffa före från-datum, se datumen i åldersgruppen %s.' %
                                                         tmp_vgroup_property['property'],
                                                         problematic_value=tmp_age_group['from_date'] + ' - ' +
                                                                           tmp_age_group['from_date'],
                                                         explanation_key='date_order')
                    ok_4 = tmp_vgroup_property['from_date'] >= visiting_group_o['from_date']
                    if not ok_4:
                        validation_error_messages.report('properties',
                                                         u'Från-datum %s som tillhör åldersgrupp %s kan inte inträffa före från-datum %s för din grupp.' % (
                                                             tmp_vgroup_property['from_date'],
                                                             tmp_vgroup_property['property'],
                                                             visiting_group_o['from_date']),
                                                         problematic_value=tmp_vgroup_property['from_date'],
                                                         explanation_key='date_range')
                    ok_5 = tmp_vgroup_property['to_date'] <= visiting_group_o['to_date']
                    if not ok_5:
                        validation_error_messages.report('properties',
                                                         u'Till-datum %s som tillhör åldersgrupp %s kan inte inträffa efter från-datum %s för din grupp.' % (
                                                             tmp_vgroup_property['to_date'],
                                                             tmp_vgroup_property['property'],
                                                             visiting_group_o['to_date']),
                                                         problematic_value=tmp_vgroup_property['to_date'],
                                                         explanation_key='date_range')
                    ...
                    if 0 == tmp_age_group['value']:
                        ...
                        ok_6, new_from_date = sanitizeDate(tmp_age_group['from_date'],
                                                           default_date=visiting_group_o['from_date'])
                        if not ok_6:
                            validation_error_messages.report('properties',
                                                             u'Från-datum som tillhör åldersgrupp %s har fel format.' %
                                                             tmp_vgroup_property['property'],
                                                             problematic_value=tmp_age_group['from_date'],
                                                             explanation_key='date_format')
                        ok_7, new_to_date = sanitizeDate(tmp_age_group['to_date'],
                                                         default_date=visiting_group_o['to_date'])
                        if not ok_7:
                            validation_error_messages.report('properties',
                                                             u'Till-datum som tillhör åldersgrupp %s har fel format.' %
                                                             tmp_vgroup_property['property'],
                        ...
                        new_property_row = {u'description': tmp_age_group['age_group'],
                                            u'value': tmp_age_group['value'], u'from_date': new_from_date,
                                            u'to_date': new_to_date, u'property': tmp_age_group['property'],
                                            u'unit': tmp_age_group['unit']}
        ...
        # TODO: We should have a two step process: first construct all bookings (make a list) and if it all is
        #  successfull and no validation errors, thats when we actually write them to the db.
        ...
        if may_change_request_data and not validation_error_messages.hasErrors():
            if 'True' == ready_to_process:
                ...
                for tmp_request in program_request_list['items']:
                    ...
                    request_for_age_groups = [x[4:] for x in
                                              ['age_sma', 'age_spar', 'age_uppt', 'age_aven', 'age_utm', 'age_rov',
                                               'age_led'] if tmp_request[x]]
                    ...
                    if len(request_for_age_groups) > 0:
                        ...
                        if not ok_r1:
                            validation_error_messages.report('booking request',
                                                             u'Det angivna önskade datumet %s har fel format.' %
                                                             tmp_request['requested_date'],
                                                             problematic_value=tmp_request['requested_date'],
                                                             explanation_key='date_format')
                        if not ((requested_date >= visiting_group_o['from_date']) and (
                                requested_date <= visiting_group_o['to_date'])):
                            validation_error_messages.report('booking request',
                                                             u'Det efterfrågade datumet %s är inte mellan din grupps från- och till- datum.' %
                                                             tmp_request['requested_date'],
                                                             problematic_value=tmp_request['requested_date'],
                                                             explanation_key='date_range')
                        ...
                        # TODO: what if there is no requested activity? TODO: how do users choose not to which
                        #  anything after they have made a request? I think we actually will need a NULL request and
                        #  this is where we among other things check the null request.
                        ...
                        # ...given fm/em/evening, look up the matching slot_id. Probably the schema will be needed (
                        # and maybe index 0,1,2,3...)
                        tmp_activity_row = schema_o['schema'][requested_activity_id]

                        # ...look through the list, lets say we have a time and we need the slot id matching that
                        # time then the reasonable way is too look through all list entries being dicts and return
                        # slot id when time match found
                        ...
                        for tmp_slot_info in tmp_activity_row[1:]:
                            if (tmp_slot_info['time_from'] < time_selection_lookup[requested_time]) and (
                                    tmp_slot_info['time_to'] > time_selection_lookup[requested_time]):
                                ...
                                new_booking = dict(type='booking', valid_from='', valid_to='',
                                                   requested_date=requested_date, slot_id=match_slot_id,
                                                   booking_day_id=booking_day_o['_id'], subtype='program')
                                ...
                                content = u'%s (önskar %s %d/%d %s) %s' % (
                                    ' '.join(['$$%s' % a for a in request_for_age_groups]),
                                    activity_selection_map.get(requested_activity_id, activity_o['title']),
                                    int(time.strftime("%d", requested_date_o)),
                                    int(time.strftime("%m", requested_date_o)),
                                    time_selection_translator.get(requested_time, requested_time), tmp_request['note'])

                                new_booking['content'] = content
                                new_booking['cache_content'] = computeCacheContent(getHollyCouch()[visiting_group_id],
                                                                                   content)
                                ...
                                remember_new_booking_request(getHollyCouch(), booking=new_booking,
