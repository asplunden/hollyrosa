...
from hollyrosa.model.booking_couch import genUID, getAllVisitingGroups, getTargetNumberOfNotesMap, getAllTags, getNotesForTarget, \
    getBookingOverview, getBookingEatOverview, getDocumentsByTag, \
    getVisitingGroupsByVodbState, getVisitingGroupsByBoknstatus, dateRange, getVisitingGroupsByGroupType, \
    getRoomBookingsOfVODBGroup, getAllActivities, getVisitingGroupTypes, getVisitingGroupsAtDate
from hollyrosa.controllers.common import workflow_map, DataContainer, getLoggedInUserId, getRenderContent, has_level
from hollyrosa.controllers.common import reFormatDate, bokn_status_map, vodb_status_map, \
    makeVODBGroupObjectOfVGDictionary, vodb_eat_times_options, vodb_live_times_options, sanitizeDate, cleanHtml
...
class VODBGroup(BaseController):
    ...
    def view_all(self):
        ...
        return dict(visiting_groups=visiting_groups,
                    ...
                    program_state_map=bokn_status_map, reFormatDate=reFormatDate, all_tags=all_tags,
                    has_notes_map=has_notes_map, visiting_group_types=getVisitingGroupTypes(getHollyCouch()),
                    state_map_list=state_map_list, vodb_state_map=bokn_status_map)
    ...
    def view_tags(self, tag):
        ...
        return dict(visiting_groups=visiting_groups,
                    remaining_visiting_group_names=remaining_visiting_groups_map.keys(),
                    bokn_status_map=bokn_status_map, vodb_state_map=bokn_status_map, state_map_list=state_map_list,
                    program_state_map=bokn_status_map, reFormatDate=reFormatDate, all_tags=all_tags,
                    has_notes_map=has_notes_map, visiting_group_types=getVisitingGroupTypes(getHollyCouch()))
    ...
    def view_program_state(self, program_state=None):
        ...
        return dict(visiting_groups=visiting_groups, remaining_visiting_group_names=v_group_map.keys(),
                    program_state_map=bokn_status_map, vodb_state_map=bokn_status_map, reFormatDate=reFormatDate,
                    all_tags=all_tags, has_notes_map=has_notes_map,
    ...
    def view_vodb_state(self, vodb_state=None):
        ...
        return dict(visiting_groups=visiting_groups, remaining_visiting_group_names=v_group_map.keys(),
                    program_state_map=bokn_status_map, vodb_state_map=bokn_status_map, state_map_list=state_map_list,
                    reFormatDate=reFormatDate, all_tags=all_tags, has_notes_map=has_notes_map,
                    visiting_group_types=getVisitingGroupTypes(getHollyCouch()))
    ...
    def view_group_type(self, group_type=None):
        # boknstatus=boknstatus[:4] # amateurish quick sanitation
        # visiting_groups = get_visiting_groups_with_boknstatus(boknstatus)
        visiting_groups = [v.doc for v in getVisitingGroupsByGroupType(getHollyCouch(), group_type)]
        v_group_map = dict()
        has_notes_map = getTargetNumberOfNotesMap(getHollyCouch())
        all_tags = [t.key for t in getAllTags(getHollyCouch())]
    ...
    @require(
        Any(has_level('pl'), has_level('pl'), has_level('staff'), msg='Only staff and pl may edit vodb group data'))
    def edit_group_data(self, visiting_group_id='', subtype='', **kwargs):
        #        visiting_group_x = getHollyCouch()[visiting_group_id]
    ...
    def save_vodb_group_properties(self,
                                   vodb_group_id='',
                                   boknr='',
                                   name='',
                                   info='',
        ...
        if is_new:
            ...
            visiting_group_o = getHollyCouch()[vgroup_id]
        ...
        # ...update properties
        visiting_group_o['visiting_group_properties'] = \
            populatePropertiesAndRemoveUnusedProperties(visiting_group_o, visiting_group_properties)

        updateBookingsCacheContentAfterPropertyChange(getHollyCouch(), visiting_group_o, getLoggedInUserId(request))
        vodb_tag_times_tags = \
            computeAllUsedVisitingGroupsTagsForTagSheet(
                visiting_group_o['tags'],
                visiting_group_o.get('vodb_tag_sheet',
                                     dict(items=[]))['items'])

        updateVisitingGroupComputedSheets(visiting_group_o, visiting_group_o['visiting_group_properties'],
    ...
    @require(
        Any(has_level('pl'), has_level('staff'), has_level('view'), msg='Only logged in users may view me properties'))
    def edit_group_sheet(self, visiting_group_id=''):
        visiting_group_o = getHollyCouch()[visiting_group_id]
        tmpl_context.form = create_edit_vodb_group_form
        ...
        empty_vodb_live_sheet = self.make_empty_vodb_live_sheet(visiting_group_o['from_date'],
                                                                visiting_group_o['to_date'])
        visiting_group_o['vodb_live_sheet'] = self.prepareAndSanitizeSheetDataForEdit(visiting_group_o,
                                                                                      'vodb_live_sheet', l_from_date,
                                                                                      l_to_date, empty_vodb_live_sheet)

        # ...same thing for eat data
        empty_vodb_eat_sheet = self.make_empty_vodb_eat_sheet(visiting_group_o['from_date'],
                                                              visiting_group_o['to_date'])
        visiting_group_o['vodb_eat_sheet'] = self.prepareAndSanitizeSheetDataForEdit(visiting_group_o, 'vodb_eat_sheet',
                                                                                     l_from_date, l_to_date,
                                                                                     empty_vodb_eat_sheet)

        # ...same thing for tag data
        tag_data = visiting_group_o.get('vodb_tag_sheet', dict(identifier='rid', items=[]))
        all_current_vgroup_tags = computeAllUsedVisitingGroupsTagsForTagSheet(visiting_group_o['tags'],
                                                                              tag_data['items'])
        empty_vodb_tag_sheet = self.make_empty_vodb_sheet(visiting_group_o['from_date'], visiting_group_o['to_date'],
                                                          vodb_live_times, all_current_vgroup_tags.keys())
        visiting_group_o['vodb_tag_sheet'] = self.prepareAndSanitizeSheetDataForEdit(visiting_group_o, 'vodb_tag_sheet',
                                                                                     l_from_date, l_to_date,
                                                                                     empty_vodb_tag_sheet)
        ...
        return dict(vodb_group=visiting_group_o, tag_layout_tags=tag_layout_tags, reFormatDate=reFormatDate,
                    vodb_state_map=bokn_status_map, program_state_map=bokn_status_map, notes=notes)

    @expose()
    @require(Any(has_level('pl'), has_level('staff'), has_level('view'),
                 msg='Only staff members may update vodb group sheets'))
    def update_group_sheets(self, vgroup_id='', tag_sheet=None, eat_sheet=None, live_sheet=None, saveButton=''):
        ...
        vodb_tag_times_tags = computeAllUsedVisitingGroupsTagsForTagSheet(visiting_group_o['tags'],
                                                                          vodb_tag_sheet['items'])
        ...
        updateVisitingGroupComputedSheets(visiting_group_o, visiting_group_properties,
                                          sheet_map=dict(vodb_eat_sheet=vodb_eat_times_options,
                                                         vodb_live_sheet=vodb_live_times_options,
                                                         vodb_tag_sheet=vodb_tag_times_tags))
    ...
    def view_vodb_group(self, visiting_group_id=''):
        ...
        visiting_group_o.show_vodb_live_sheet = json.dumps(
            dict(identifier='rid', items=visiting_group_o.get('vodb_live_computed', list())))
        visiting_group_o.show_vodb_eat_sheet = json.dumps(
            dict(identifier='rid', items=visiting_group_o.get('vodb_eat_computed', list())))
        visiting_group_o.show_vodb_tag_sheet = json.dumps(
            dict(identifier='rid', items=visiting_group_o.get('vodb_tag_computed', list())))
        ...
        room_bookings = list()
        for b in getRoomBookingsOfVODBGroup(getHollyCouch(), visiting_group_id):
            # ...lookup slot map
            live_schema_id = 'room_schema.2013'  # TODO: Read dynamically from booking day
            tmp_slot_row_data = list(getHollyCouch().view('day_schema/slot_map', keys=[[b['slot_id'], live_schema_id],
                                                                                       [b['booking_end_slot_id'],
                                                                                        live_schema_id]]))

            start_time = tmp_slot_row_data[0].value[1]['time_from']
            end_time = tmp_slot_row_data[1].value[1]['time_to']
            b2 = DataContainer(booking_state=b['booking_state'], cache_content=b['cache_content'], content=b['content'],
                               activity=activities[b['activity_id']], id=b['_id'],
                               booking_date=b.get('booking_date', 'Unknown'),
                               booking_end_date=b.get('booking_end_date', 'Unknown'),
                               booking_day_id=b.get('booking_day_id', ''), valid_from=b.get('valid_from', ''),
                               valid_to=b.get('valid_to', ''), requested_date=b.get('requested_date', ''),
                               start_time=start_time, end_time=end_time)

            room_bookings.append(b2)

        tag_layout_tags = json.dumps(visiting_group_o['tags'])
        return dict(visiting_group=visiting_group_o, reFormatDate=reFormatDate, vodb_state_map=bokn_status_map,
                    program_state_map=bokn_status_map, notes=notes, tag_layout_tags=tag_layout_tags,
                    room_bookings=room_bookings, getRenderContent=getRenderContent)
    ...
    def edit_request(self, visiting_group_id=''):
        ...
        program_request_data_dict = {'identifier': 'id', 'items': [[{'id': 0,
                                                                     'requested_date': visiting_group_o['from_date'],
                                                                     'requested_time': '', 'requested_activity': '',
                                                                     'age_sma': False, 'age_spar': False,
                                                                     'age_uppt': False, 'age_aven': False,
                                                                     'age_utm': False, 'age_rov': False,
                                                                     'age_led': False, 'note': ''} for i in range(35)]]}
        ...
        return dict(visiting_group_program_request=visiting_group_o, reFormatDate=reFormatDate,
    ...
    def vodb_eat_overview(self, compute_local_sum=False, compute_live=False):
        if compute_live:
            overview_live_o = getBookingOverview(getHollyCouch(), None, None, reduce=False)
        else:
            overview_live_o = list()
        overview_eat_o = getBookingEatOverview(getHollyCouch(), None, None, reduce=False)
        ...
        for row in list(overview_live_o) + list(overview_eat_o):
            ...
            if compute_local_sum:
                try:
                    overview_value_map[
                        '%s:%s:%s:%s' % (row.id, row.key[0], row.key[1], 'SUM')] = overview_value_map.get(
                        '%s:%s:%s:%s' % (row.id, row.key[0], row.key[1], 'SUM'), 0) + int(row.value)
                except ValueError:
                    pass
                except TypeError:
                    pass
        ...
        status_level_overview_o = getBookingOverview(getHollyCouch(), None, None, reduce=True)
        status_level_eat_overview = getBookingEatOverview(getHollyCouch(), None, None, reduce=True)
        # ...copy paste!
        for row in list(status_level_overview_o) + list(status_level_eat_overview):
            ...
            if compute_local_sum:
                try:
                    overview_value_map[
                        '%s:%s:%s:%s' % (tmp_id, row.key[0], row.key[1], 'SUM')] = overview_value_map.get(
                        '%s:%s:%s:%s' % (tmp_id, row.key[0], row.key[1], 'SUM'), 0) + int(row.value)
                except ValueError:
                    pass
        ...
        return dict(header_dates=header_dates, header_times=header_times, row_choices=row_choices, vgroup_opts=vgroups,
                    values=overview_value_map)
        ...
        for live_option in a_options:
            for vodb_status, vodb_status_name in a_vodb_status_map.items():
                try:
                    ...
                    summary_live_option_vgroups = summary_vgroups[live_option]
                summary_live_option_vgroups[vodb_status] = DataContainer(
                    id='summary_%s_%d' % (live_option, vodb_status), name=vodb_status_name, vodbstatus=vodb_status,
                    from_date='', to_date='', vodb_live_computed=list(), has_values=False, all_values_zero=True)
        ...
        for live_option in a_vodb_live_times_options:
            for vodb_status, vodb_status_name in a_vodb_status_map.items():
                ...
                if not tmp_summary_group.has_values:
                    ...
                    formated_dates = dateRange(tmp_summary_group['from_date'], tmp_summary_group['to_date'],
                                               format='%Y-%m-%d')
    ...
    def vodb_booking_overview(self, compute_local_sum=False, compute_live=False):
        ...
        # ...iterating through reduced result set should give the data we need
        live_summaries_rows = getBookingOverview(getHollyCouch(), None, None, reduce=True)
        ...
        for tmp_id in vgroup_ids.keys():
            tmp_vgroup = getHollyCouch()[tmp_id]
        ...
        return dict(header_dates=header_dates, header_times=header_times, vgroups=vgroups,
                    bokn_status_map=bokn_status_map, vodb_status_map=vodb_status_map, summary_vgroups=summary_vgroups)

    @expose()
    @validate(validators={'visiting_group_id': validators.UnicodeString(not_empty=True),
                          'live': validators.UnicodeString(not_empty=True),
                          'change_schema': validators.UnicodeString()})
    ...
    def create_calculation_schema(self, visiting_group_id=None, live='outdoor', change_schema='live'):
        ...
        # this one is more tricky since we dont want to have arrival and departure data set every day.
        elif change_schema == 'eat':
        ...
        for tmp_date in self.dateGen(from_date, to_date):
            ...
            for t in time_row_schema:
                ...
                for tmp_prop in visiting_group_properties.values():
                    if tmp_prop['from_date'] <= tmp_date <= tmp_prop['to_date']:
        ...
        vodb_tag_times_tags = computeAllUsedVisitingGroupsTagsForTagSheet(visiting_group_o['tags'],
                                                                          visiting_group_o['vodb_tag_sheet']['items'])
