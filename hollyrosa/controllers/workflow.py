...
class Workflow(BaseController):
    ...
    def overview(self):
        ...
        return dict(scheduled_bookings=scheduled_bookings,  unscheduled_bookings=unscheduled_bookings,  workflow_map=workflow_map,  activity_map=getActivityTitleMap(getHollyCouch()),  booking_date_map=getBookingDayInfoMap(getHollyCouch()),  user_name_map=getUserNameMap(getHollyCouch()), slot_map=getSchemaSlotActivityMap(getHollyCouch(), 'day_schema.1'),  workflow_submenu=workflow_submenu)
    ...
    def view_nonapproved(self):
        ...
        return dict(scheduled_bookings=scheduled_bookings,  workflow_map=workflow_map, result_title='Unapproved scheduled bookings',  activity_map=getActivityTitleMap(getHollyCouch()), booking_date_map=getBookingDayInfoMap(getHollyCouch()), user_name_map=getUserNameMap(getHollyCouch()),  slot_map=getSchemaSlotActivityMap(getHollyCouch(), 'day_schema.1'), workflow_submenu=workflow_submenu)

    @expose('hollyrosa.templates.workflow_view_scheduled')
    def view_preliminary(self):
        scheduled_bookings = [b.doc for b in gelAllBookingsWithBookingState(getHollyCouch(), [0])]
        return dict(scheduled_bookings=scheduled_bookings,  workflow_map=workflow_map, result_title='Preliminary scheduled bookings',  activity_map=getActivityTitleMap(getHollyCouch()), booking_date_map=getBookingDayInfoMap(getHollyCouch()), user_name_map=getUserNameMap(getHollyCouch()), slot_map=getSchemaSlotActivityMap(getHollyCouch(), 'day_schema.1'), workflow_submenu=workflow_submenu)

    @expose('hollyrosa.templates.workflow_view_scheduled')
    def view_scheduled(self):
        scheduled_bookings = [b.doc for b in getAllScheduledBookings(getHollyCouch())]
        return dict(scheduled_bookings=scheduled_bookings, workflow_map=workflow_map, result_title='Schedueld bookings',  activity_map=getActivityTitleMap(getHollyCouch()), booking_date_map=getBookingDayInfoMap(getHollyCouch()), user_name_map=getUserNameMap(getHollyCouch()), slot_map=getSchemaSlotActivityMap(getHollyCouch(), 'day_schema.1'), workflow_submenu=workflow_submenu)


    @expose('hollyrosa.templates.workflow_view_scheduled')
    def view_disapproved(self):
        scheduled_bookings = scheduled_bookings = [b.doc for b in gelAllBookingsWithBookingState(getHollyCouch(), [-10])]
        return dict(scheduled_bookings=scheduled_bookings,   workflow_map=workflow_map, result_title='Disapproved bookings', activity_map=getActivityTitleMap(getHollyCouch()), booking_date_map=getBookingDayInfoMap(getHollyCouch()), user_name_map=getUserNameMap(getHollyCouch()), slot_map=getSchemaSlotActivityMap(getHollyCouch(), 'day_schema.1'), workflow_submenu=workflow_submenu)


    @expose('hollyrosa.templates.workflow_view_unscheduled')
    def view_unscheduled(self):
        unscheduled_bookings = [b.doc for b in getAllUnscheduledBookings(getHollyCouch())]
        return dict(unscheduled_bookings=unscheduled_bookings,  workflow_map=workflow_map, result_title='Unscheduled bookings', activity_map=getActivityTitleMap(getHollyCouch()), booking_date_map=getBookingDayInfoMap(getHollyCouch()), user_name_map=getUserNameMap(getHollyCouch()), slot_map=getSchemaSlotActivityMap(getHollyCouch(), 'day_schema.1'), workflow_submenu=workflow_submenu)
...
#                raise redirect(request.referrer)
    ...
    def set_state(self, booking_id=None,  state=0, all=0):
        ...
        if all == 0 or all==None:
            ...
            booking_o = getHollyCouch()[booking_id]
            bookings = [b.doc for b in getAllSimilarBookings(getHollyCouch(), [booking_o['visiting_group_id'], booking_o['activity_id']]) ] # TODO: Fix later DBSession.query(booking.Booking).filter(and_('visiting_group_id='+str(booking_o.visiting_group_id), 'activity_id='+str(booking_o.activity_id), 'booking_state > -100')).all()
