...
class DateConverter(FancyValidator):
    ...
    #mchange
    _day_date_re_eu = re.compile(r'^\s*(\d\d\d?\d?)[\-\./\\](\d\d?|jan|january|feb|febuary|mar|march|apr|april|may|jun|june|jul|july|aug|august|sep|sept|september|oct|october|nov|november|dec|december)[\-\./\\](\d\d?)\s*$', re.I)
    
    _month_date_re_eu = re.compile(r'^\s*(\d\d\d?\d?)[\-\./\\](\d\d?|jan|january|feb|febuary|mar|march|apr|april|may|jun|june|jul|july|aug|august|sep|sept|september|oct|october|nov|november|dec|december)[\-\./\\]\s*$', re.I)
    #end mchange
    ...
    def __init__(self, *args, **kw):
        ...
        if not self.month_style in ('dd/mm/yyyy', 'mm/dd/yyyy',  'yyyy-mm-dd'):
    ...
    def convert_day(self, value, state):
        ...
        #mchange to EU
        if self.month_style == 'yyyy-mm-dd':
            match = self._day_date_re_eu.search(value)
        else:
        ...
        #mchange
        if self.month_style == 'yyyy-mm-dd':
            year = self.make_year(match.group(1), state) 
        else:
    ...
    def convert_month(self, value, state):
        # mchange
        if self.month_style == 'yyyy-mm-dd':
            match = self._month_date_re_eu.search(value)
        else:
    ...
    def unconvert_day(self, value, state):
        ...
        elif self.month_style == 'yyyy-mm-dd':
