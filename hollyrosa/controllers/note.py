...
class Note(BaseController):
    ...
    @expose("json")
    @require(Any(is_user('user.erspl'), has_level('staff'), has_level('view'),
                 msg='Only staff members and viewers may view visiting group properties'))
    ...
    def save_note(self, target_id, note_id, text):
        ...
        if note_id == '' or note_id is None:
            note_o = dict(type='note', note_id=genUID(type='note'), target_id=target_id, note_state=0, tags=list(),
                          history=list(), text='')
            note_change = 'new'
    ...
    def save_attachment(self, target_id, text, attachment, _id='', **kwargs):
        ...
        timestamp = datetime.datetime.now().strftime('%Y-%m-%d %H:%M')
        if _id == '':
            attachment_o = dict(type='attachment', _id=genUID(type='attachment'), target_id=target_id,
                                attachment_state=0, tags=list(), history=list(), text='')
        ...
        # TODO FIX BELOW remember_attachment_change(getHollyCouch(), target_id=target_id, attachment_id=attachment_o[
    ...
    @require(Any(has_level('pl'), has_level('staff'), has_level('view'),
                 msg='Only staff members may view visiting group attachments'))
    def download_attachment(self, attachment_id, doc_id):
        ...
        headers = ('Content-Disposition',
                   ('attachment;filename=%s' % doc_id).replace(u' ', u'_').replace(u'å', u'a').replace(u'ö',
                                                                                                       u'o').encode(
                       'ascii', 'replace'))
