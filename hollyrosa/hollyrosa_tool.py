...
# ...We will have to list all activities and all activity_groups and then look for all booking_info_id. Save them all
# in a dict and after that we can delete note that are not in that dict ...better mark the docs as eternal
...
def makeSlotRow(slot_id_start, zorder, activity_id):
    ...
    result.append({u'duration': u'12:00:00', u'time_to': u'12:00:00', u'slot_id': u'slot.%d' % slot_id_start,
                   u'time_from': u'00:00:00', u'title': u'FM'})
    slot_id_start += 1
    result.append({u'duration': u'12:00:00', u'time_to': u'23:59:59', u'slot_id': u'slot.%d' % slot_id_start,
                   u'time_from': u'12:00:00', u'title': u'EM'})
    slot_id_start += 1
...
def makeRoom(holly_rosa, title='', activity_group_id='', capacity='', zorder=0, schema=None, max_slot_id=0):
    new_room = dict(tags="", certificate_needed=None, bg_color='#fff', capacity=0, title='-', type="activity", zorder=0,
                    do_not_delete=True, default_booking_state=0, subtype='room')
...
if False:
    ...
    makeRoom(holly_couch, 'Enskede/Prästgården - Vardagsrum', activity_group_id='roomgroup.skrakvik', capacity=2,
             zorder=84)
    makeRoom(holly_couch, 'Enskede/Prästgården - Vänster', activity_group_id='roomgroup.skrakvik', capacity=2,
             zorder=85)
...
if False:
    ...
    max_slot_id = makeRoom(holly_couch, 'Kökslags chef', activity_group_id='funkgroup.kok', capacity=0, zorder=200,
                           schema=schema, max_slot_id=max_slot_id)
    max_slot_id = makeRoom(holly_couch, 'Kök', activity_group_id='funkgroup.kok', capacity=0, zorder=201, schema=schema,
                           max_slot_id=max_slot_id)
    max_slot_id = makeRoom(holly_couch, 'Inten PL', activity_group_id='funkgroup.inten', capacity=0, zorder=202,
                           schema=schema, max_slot_id=max_slot_id)
    max_slot_id = makeRoom(holly_couch, 'Inten', activity_group_id='funkgroup.inten', capacity=0, zorder=203,
                           schema=schema, max_slot_id=max_slot_id)
    max_slot_id = makeRoom(holly_couch, 'Bagar PL', activity_group_id='funkgroup.bagare', capacity=0, zorder=204,
                           schema=schema, max_slot_id=max_slot_id)
    max_slot_id = makeRoom(holly_couch, 'Bagare', activity_group_id='funkgroup.bagare', capacity=0, zorder=205,
                           schema=schema, max_slot_id=max_slot_id)
    max_slot_id = makeRoom(holly_couch, 'Lillis PL', activity_group_id='funkgroup.lillis', capacity=0, zorder=206,
                           schema=schema, max_slot_id=max_slot_id)
    max_slot_id = makeRoom(holly_couch, 'Lillis', activity_group_id='funkgroup.lillis', capacity=0, zorder=207,
                           schema=schema, max_slot_id=max_slot_id)
    max_slot_id = makeRoom(holly_couch, 'Skeppare', activity_group_id='funkgroup.rederiet', capacity=0, zorder=208,
                           schema=schema, max_slot_id=max_slot_id)
    max_slot_id = makeRoom(holly_couch, 'Matros', activity_group_id='funkgroup.rederiet', capacity=0, zorder=209,
                           schema=schema, max_slot_id=max_slot_id)
    max_slot_id = makeRoom(holly_couch, 'Assistent', activity_group_id='funkgroup.island', capacity=0, zorder=210,
                           schema=schema, max_slot_id=max_slot_id)
    max_slot_id = makeRoom(holly_couch, 'Arbetshäst', activity_group_id='funkgroup.island', capacity=0, zorder=211,
                           schema=schema, max_slot_id=max_slot_id)
    max_slot_id = makeRoom(holly_couch, 'Lägerdoktor', activity_group_id='funkgroup.island', capacity=0, zorder=212,
                           schema=schema, max_slot_id=max_slot_id)
    max_slot_id = makeRoom(holly_couch, 'Sjöledare', activity_group_id='funkgroup.fladan', capacity=0, zorder=213,
                           schema=schema, max_slot_id=max_slot_id)
    max_slot_id = makeRoom(holly_couch, 'Fladian', activity_group_id='funkgroup.fladan', capacity=0, zorder=214,
                           schema=schema, max_slot_id=max_slot_id)
    max_slot_id = makeRoom(holly_couch, 'Programledare', activity_group_id='funkgroup.program', capacity=0, zorder=215,
                           schema=schema, max_slot_id=max_slot_id)
    max_slot_id = makeRoom(holly_couch, 'Programmare', activity_group_id='funkgroup.program', capacity=0, zorder=216,
                           schema=schema, max_slot_id=max_slot_id)
    max_slot_id = makeRoom(holly_couch, 'Läger', activity_group_id='funkgroup.program', capacity=0, zorder=217,
                           schema=schema, max_slot_id=max_slot_id)
    max_slot_id = makeRoom(holly_couch, 'Traktor chef', activity_group_id='funkgroup.traktor', capacity=0, zorder=218,
                           schema=schema, max_slot_id=max_slot_id)
    max_slot_id = makeRoom(holly_couch, 'Traktor', activity_group_id='funkgroup.traktor', capacity=0, zorder=219,
                           schema=schema, max_slot_id=max_slot_id)
    max_slot_id = makeRoom(holly_couch, 'Storgårn PL', activity_group_id='funkgroup.storgarn', capacity=0, zorder=220,
                           schema=schema, max_slot_id=max_slot_id)
    max_slot_id = makeRoom(holly_couch, 'Storgårn', activity_group_id='funkgroup.storgarn', capacity=0, zorder=221,
                           schema=schema, max_slot_id=max_slot_id)
